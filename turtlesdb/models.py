...
class TrtObservations(models.Model):
    ...
    def __str__(self):
        if self.observation_status:
            return f'{self.observation_id} ({self.get_observation_datetime().isoformat()}) {(self.observation_status)}'
        else:
            return f'{self.observation_id} ({self.get_observation_datetime().isoformat()})'

    def get_observation_datetime(self):
        """Returns a combined observation datetime, in AWST.
        """
        if self.observation_time:
            return datetime(self.observation_date.year, self.observation_date.month, self.observation_date.day, self.observation_time.hour, self.observation_time.minute, tzinfo=AWST)
        else:
            return datetime(self.observation_date.year, self.observation_date.month, self.observation_date.day, 0, 0, tzinfo=AWST)
